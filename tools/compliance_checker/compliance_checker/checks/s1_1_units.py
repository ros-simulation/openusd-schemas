...
def _check_meters_per_unit(stage):
...
def _check_kilograms_per_unit(stage):
...
def _check_time_codes_per_second(stage):
...
def _check_root_rotation(stage):
...
def _check_kinematic_transform_ops(stage):
    errors = []
...
def _check_kinematic_scale(stage):
    errors = []
    for prim in stage.TraverseAll():
        if not _is_kinematic_prim(prim):
            continue
...
def _validate_coordinate_system(stage: Usd.Stage, timeRange: TimeRange) -> list:
    errors = []
